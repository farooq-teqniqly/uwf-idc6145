...
Recent work by Riley et al. (2020) proposes an integrated optimization and machine learning framework to improve real-time ride sharing dispatch using NYC trip data, reducing average passenger wait times by more than 50\% \cite{riley2020dispatching}. Similarly, Ma et al. introduce a congestion-aware dispatch and charging scheduling system for electric ride-hailing fleets, achieving improved profitability and service efficiency in high-congestion scenarios \cite{ma2024congestionaware}. These studies highlight advanced data-driven approaches to mitigating ride-hailing congestion and support the potential for predictive modeling grounded in large-scale NYC mobility data.
...
Several works have used this data set to assess the impact of ride-hailing on traffic flow and use of public transit. For example, a study by the NYU Tandon School of Engineering found that the New York City congestion surcharge reduced the number of ride-hailing trips, but did not significantly alleviate congestion \cite{nyu2023}. Furthermore, research from the CUNY Graduate Center examined the effects of the 2019 congestion surcharge on FHV usage in Manhattan, showing a decline in rides in charged zones \cite{cuny2023}.
